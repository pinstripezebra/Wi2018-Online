...
class donor:
    ...
    def __init__(self, name_request, donation_amount, donations = 1):
...
class donor_storage(object):
    ...
    #Takes a multiplier factor and returns a new donation_object_storage list
    #with donation values factor times greater than original
    def challenge(self, factor, min_donation = 0, max_donation = 100000, projection = False):
        if factor > 1:
            donation_amount = []
            if projection == False:
                for i in range(len(self.donor_object_storage)):
                    donation_amount.append(self.donor_object_storage[i])
            
                #Filtering list by min and max donation amounts
                filtered = list(filter(lambda x: min_donation < x.donation < max_donation, donation_amount[:]))
                #Stores donors whose donations dont fall between min/max value
                excluded_donors =  []
                for i in range(len(self.donor_object_storage)):
                    if self.donor_object_storage[i] not in filtered:
                        excluded_donors.append(self.donor_object_storage[i])
            
                #Mapping existing donation vale to a value factor times greater 
                mapped_donations = map(lambda x: x.donation * factor, filtered)
                new_amounts = list(mapped_donations)
                print(new_amounts[:])
            
                for i in range(len(filtered)):
                    filtered[i].donation = new_amounts[i]
                
                self.donor_object_storage = filtered
                self.donor_object_storage.extend(excluded_donors)
                
            #If we are dealing with a projection
            else:
                for i in range(len(self.donor_object_storage)):
                    donation_amount.append(self.donor_object_storage[i])
                
                filtered_projection = list(filter(lambda x: min_donation < x.donation < max_donation, donation_amount[:]))
                projected = list(map(lambda x: x.donation * factor, filtered_projection))
              
                self.difference = sum(projected) - sum(filtered_projection.donation for filtered_projection in filtered_projection)
    ...
    while userAction !="6":
        ...
        try:
            ...
            #
                ...
                factor = int(input("Please input philanthropic multiplier: "))
                donation_bounds = input("Would you like to set a donation range? (y/n): ")
                if donation_bounds == "y":
                    upper = int(input("Please specify an upper limit: "))
                    lower = int(input("Please specify a lower limit: "))
                    overall_storage.challenge(factor, lower, upper)
                    
                elif donation_bounds == "n":
                    overall_storage.challenge(factor)
                ...
                if donation_bounds == "y":
                    upper = int(input("Please specify an upper limit: "))
                    lower = int(input("Please specify a lower limit: "))
                    overall_storage.challenge(factor, lower, upper, projection = True)
                elif donation_bounds == "n":
                    overall_storage.challenge(factor, projection = True)
